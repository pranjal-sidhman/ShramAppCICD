...
import com.uvk.shramapplication.databinding.FragmentAcceptedJobBinding
...
class AcceptedJobFragment : Fragment() {
    private var _binding: FragmentAcceptedJobBinding? = null
    ...
    private lateinit var jobAdapter: AcceptedJobsAdapter
    val TAG = "AcceptedJobFragment"

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        _binding = FragmentAcceptedJobBinding.inflate(inflater, container, false)
    ...
    private fun getJobAcceptedList() {
        try {
            if (requireActivity().isOnline) {

                viewModel.getJobAcceptListResult.observe(requireActivity()) { response ->
                    ...
                    when (response) {
                        is BaseResponse.Success -> {
                            ...
                            if (response.data?.data.isNullOrEmpty()) {
                                ...
                                jobAdapter = AcceptedJobsAdapter(
                                    requireContext(),
                                    postJobList
                                )
                ...
                viewModel.getJobAcceptList(requireActivity().userid)
